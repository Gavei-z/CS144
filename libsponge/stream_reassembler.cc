...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    // is before the `_next_index`, do NOTHING.
    if (index >= _next_index + _capacity || _next_index > index + data.size()) return;

    // Here we should consider the situation that we shoud accept part of
    // the string, when `index < _next_index`. We should set the `actual_index`
    ...
    // Corner case: when the data is empty.
    if (data.empty())
        if (eof) _should_eof = eof;

    // We store the byte into the `_stream[index]` and set the `_st[index]`
    // to be true, if `_st[index] == false`.
    ...
    // We should calculate consecutive `_st[index]` from start.
    // Pay attention, you shouldn't set the `_st[index]` to false,
    // because we DO NOT KNOW the ByteStream's size, so we
    // need to get the bytes actually written into the `_output`.
    string send_str{};
    for (size_t i = start_index; _st[i]; i = next(i)) {
        send_str.push_back(_stream[i]);

        if (next(i) == start_index) break;
    }
    ...
    // When we got the eof and all the bytes were assembled,
    // The ByteStream should set the flag `_input_end` to be true
